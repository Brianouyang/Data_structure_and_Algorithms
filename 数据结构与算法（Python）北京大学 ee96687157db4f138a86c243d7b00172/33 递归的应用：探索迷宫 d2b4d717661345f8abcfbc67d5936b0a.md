...
- 将海龟放在迷宫中间，如何能够找到出口
- 首先，我们将整个迷宫的空间分为行列整齐的方格，区分出墙壁和通道

    给每个方格具有行列位置，并赋予“墙壁”、“通道”属性
    ...
    - 这样，探索迷宫的递归算法思路如下：
        ...
        如果上面的不中找不到出口，那么将海龟从位置向南移动一步，以新位置递归调用探索迷宫

        如果上面的不中找不到出口，那么将海龟从位置向西移动一步，以新位置递归调用探索迷宫

        如果上面的不中找不到出口，那么将海龟从位置向东移动一步，以新位置递归调用探索迷宫

        如果找不到出口，那么这个迷宫没有出口

        细节：

        如果某个方向移动了海龟，那么如果新位置的北正好是一堵墙壁，那么新位置上海龟会向南移动，可是新位置的南边一格，正好就是递归调用之前的原位置，这样会陷入无限递归的死循环

    - 解决：

        沿路撒面包屑，一旦发现“面包屑”，就不从那走了
